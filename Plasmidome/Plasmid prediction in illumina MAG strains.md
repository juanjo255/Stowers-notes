...
5. **RFPlasmid:** Similar to Plasmer with limitations due to the dataset used during training, I trained my own model.
...
2. **Query cover:** Given that this is Illumina we can have cases where the gene indel is at the beginning or end of the read. I have noticed that between the mega-plasmids the changes can be enormous >10kb, the following example does not belong to an accessory plasmid:[example_variable_psymA.png](https://webfs/n/projects/jp2992/stowers-notes/photos/example_diff_psymA.png)
	* However, we can see that using a cov>90%, this giant contig that belongs to the super-plasmids is discarded because the best reference for it is lacking 39kb at the beginning and 10kb at the end. Resulting in a query coverage of 89%.

**Therefore, I decided that a query cov. > 70 %  for aligned fragments 2kb< x < 50 kb. For x>50kb only identity is taken into account**.  
...
> [!NOTE]
> **This strategy will not reconstruct the accessory plasmids**, but, as long as we have enough reference genomes, it will tell us unique accessory contigs. **The real problem is the mega-plasmids, like psymA.** It carries accessory genes or genomic islands. ***How can we differentiate an accessory gene coming from psymA or the accessory plasmid?***
> 

The presence of accessory genes provided by an accessory plasmid can be confirmed with plasmid proteins predictions, for example, relaxases, MPF and replicons by ***Mob-typer***. All genomes having more than 2 relaxases (or MPF, the problem with MPF is that it is not well annotated for these plasmids) matches might indicate the presence of accessory plasmids.
...
The *table* column shows which of the tables below presented these strains (i.e. they were predicted) and the *acc. plasmid* column shows if they present accessory plasmid according to the assembly. Only, MAG230 was among our MAG collection and it presents an acc. plasmid, but it was not predicted to have acc. pls. 
...
**In bold** we see the in-house strains sequenced with PacBio that presented an accessory plasmid. The sizes represents the "unique" contigs found according to the first 

\* It did not present accessory plasmid according to sam.

> [!UPDATE]
> 
> After prediction, Samantha found "that **only MAG748A did not have one**. MAG18, MAG97, and MAG342 had two accessory plasmids, and the rest had one."
...
# Second Result: prediction by machine-learning based methods
...
## Prediction with MGE-Cluster (Clustering strategy)
...
> [!NOTE]
> This did not worked very well by itself :( however, I think I can improved it. We sequence strains predicted by the model to have plasmids which were not identified by the other processes (13). However, 11 were false positive. I will re-trained the model with the new data but this time I will include something I should have included before. **I will remove the k-mers shared between plasmid and chromosome.** I will try to do it and feed this model with it, otherwise I will try to feed it to Plasmer which uses this strategy.
> 

This tools allow me to train a random forest model from chromosomes and plasmids. The recommendation is to have at least 20 sequences of each as well as draft genomes.

This is the most laborious part here. I need to establish a very good dataset. 
...
I will use as my positive control test dataset all the predicted at the beginning of this document (n=20) where Samantha confirmed 19.
 

From the 30 strains that remained unpredicted from the first result. **A total of 13 more were retrieved** according to their plasmids predictions. 
...
These strains will be sequenced. 


# Results from sequencing

I received 48 strains. 13 prediction were made by the model for the un-typable strains and 11 were false positive. 

This result might reveal a disappointed aspect from this process. The accessory plasmids might not be separable by k-mer (RFPlasmid uses 5mers). This means that most reliable part of the pipeline comes from checking the number of relaxases and the RepABC module. The problem with this part is that it relies on databases to annotate these proteins.
